      ...
      <div className="flex flex-col md:flex-row gap-8 items-start">
        ...
        <div className="w-full md:w-1/2">
          ...
          <div className="flex gap-4 justify-center">
            <button
              disabled={!projects[currentIndex].demoUrl}
              className={`group relative px-6 py-2 rounded-lg overflow-hidden transition-all duration-300 ${
                projects[currentIndex].demoUrl
                  ? 'bg-blue-600 text-white hover:bg-blue-700 cursor-pointer'
                  : 'bg-gray-700 text-gray-400 cursor-not-allowed'
              }`}
              onClick={() => projects[currentIndex].demoUrl && window.open(projects[currentIndex].demoUrl, '_blank')}
            >
              <span className="relative z-10 flex items-center gap-2">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                </svg>
                {projects[currentIndex].demoUrl ? 'Live Demo' : 'Coming Soon'}
              </span>
              {projects[currentIndex].demoUrl && (
            ...
            <button
              disabled={!projects[currentIndex].githubUrl}
              className={`group relative px-6 py-2 rounded-lg overflow-hidden transition-all duration-300 ${
                projects[currentIndex].githubUrl
                  ? 'bg-gray-800 text-white hover:bg-gray-700 cursor-pointer'
                  : 'bg-gray-700 text-gray-400 cursor-not-allowed'
              }`}
              onClick={() => projects[currentIndex].githubUrl && window.open(projects[currentIndex].githubUrl, '_blank')}
            >
              <span className="relative z-10 flex items-center gap-2">
                ...
                {projects[currentIndex].githubUrl ? 'GitHub' : 'Coming Soon'}
              </span>
              {projects[currentIndex].githubUrl && (
